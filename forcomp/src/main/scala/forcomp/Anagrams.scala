...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.map(_.toLower).toSeq.groupBy(identity).toList.sortBy(_._1).map { case char -> chars => char -> chars.length }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =
    s.flatMap(wordOccurrences)
      .groupBy { case char -> count => char }
      .toList
      .sortBy(_._1)
      .map { case char -> pairs =>
        val sumOfCount = pairs.map { case char -> count => count }.sum
        char -> sumOfCount
      }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    combinationsAux(occurrences).map(_.toList)

  def combinationsAux(occurrences: Occurrences): List[collection.SortedMap[Char, Int]] =
    occurrences match {
      case Nil => List(collection.SortedMap[Char, Int]())
      case char -> count :: tail =>
        val assumption = combinationsAux(tail)
        (1 to count).flatMap { newCharCount =>
          assumption.map { occurrences =>
            occurrences + (char -> newCharCount)
          }
        }.toList ++ assumption
    }
