...
abstract class TweetSet {
   ...
   */
   def union(that: TweetSet): TweetSet = {
     var newThis = this
     that.foreach(thatTweet => newThis = newThis.incl(thatTweet))
     newThis
   }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet =
      if(p(elem))
        new NonEmpty(elem, left.filter(p), right.filterAcc(p, acc))
      else
        left.filterAcc(p, right.filterAcc(p, acc))

  def mostRetweeted: Tweet = {
    var candidates = List(elem)
    try candidates = candidates.appended(left.mostRetweeted) catch {
      case _: java.util.NoSuchElementException => ()
    }
    try candidates = candidates.appended(right.mostRetweeted) catch {
      case _: java.util.NoSuchElementException => ()
    }
